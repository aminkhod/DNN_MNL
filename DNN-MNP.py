...
class Dense1(Layer):
    ...
    def call(self, inputs, **kwargs):
        ...
        out1=tf.matmul(v1,tf.ones((1, self.iternum), tf.float32))+ error[:,0]
        


        v2 =tf.reshape( (self.W1 * inputs[:,4]) + (self.W2 * inputs[:,5]) + (self.W3 * inputs[:,6])+ (self.W4 * inputs[:,7]),(self.batch,1))

        out2=tf.matmul(v2,tf.ones((1, self.iternum), tf.float32))+ error[:,1]
...
mergedOutput=tf.reshape(mergedOutput1,(batch,2,iternum))
